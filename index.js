...
const app = express();
const port = process.env.PORT || 3001; // Must be different than the port of the React app
mongoose.connect(
  "mongodb+srv://root:" +
    process.env.MONGODB_PWD +
    "@cluster0.jd3p4fo.mongodb.net/myFirstDb?retryWrites=true&w=majority",
  {
...
app.get("/grabbeditems/:userInfo", async (req, res) => {
  const grabbeditems = await grabbeditemModel.find({
    grabber_name: req.params.userInfo,
  });
  res.send(grabbeditems);
});

app.get("/wisheditems/:userInfo", async (req, res) => {
  const wisheditems = await wisheditemModel.find({
    grabber_name: req.params.userInfo,
  });
  console.log(req.body.userInfo);
...
app.delete("/wisheditems", async (req, res) => {
  const results = await wisheditemModel.deleteOne({
    wisheditem_id: req.body.id,
  });
  res.send(results);
});
...
app.post("/users/login", async (request, response) => {
  ...
  response.send({ success: false, msg: "username or password is empty" });
});
